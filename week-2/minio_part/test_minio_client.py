...
def test_bucket_exsit(minio_client_native: MinioClientNative):
    bucket_name = "week2"
    assert minio_client_native.client.bucket_exists(bucket_name) == True


def test_upload_file(minio_client_native: MinioClientNative, file_to_save: Path, tmp_path: Path):
    bucket_name = "week2"
    if minio_client_native.client.bucket_exists(bucket_name):
